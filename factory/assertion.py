...
from factory.strings_util import StringUtil as String
...
class Assertion(str):
    ...
    def is_equals_to(
        ...
        expected = (
            str(value_expected).replace("_RANDOM_DATA_", random_data.get()).strip()
    ...
    def is_in_strings_list(self, list_of_strings_expected, optional_description=""):
        ...
        try:
            ...
            Log.success(f"{optional_description} --> List checked successfully")
        except AssertionError as ae:
            Rexc.raise_assertion_error(
                f"(X) {optional_description} --> There are missed element between the lists",
                ae,
            )
