...
from factory.text_colour_util import TextColour as Colour
...
class BaseLogging:
    ...
    def display_debug_error(message):
        ...
        logging.error(Colour.red("   ERROR: {}".format(message)), exc_info=True)
    ...
    def show_response(response, show_header=False):
        try:
            if show_header:
                ...
                print(Colour.blue(response.headers) + "\n")
            parsed_body = json.loads(str(response.text))
            BaseLogging.info("-- RESPONSE BODY --")
            print(Colour.blue(StringUtil.indent_dict_body(parsed_body)))
